...
export const metadata = {
	title: "Shopping Cart · Node Runner",
};
...
async function CartContent({ params: paramsPromise }: { params: Promise<{ channel: string }> }) {
	...
	if (!checkout || checkout.lines.length < 1) {
		return (
			<div className="mt-12">
				...
				<LinkWithChannel
					href="/products"
					className="hover:bg-primary/90 inline-block max-w-full rounded border border-transparent bg-primary px-6 py-3 text-center font-medium text-primary-foreground aria-disabled:cursor-not-allowed aria-disabled:opacity-50 sm:px-16"
				>
					Explore products
				</LinkWithChannel>
			</div>
	...
	return (
		<form className="mt-12">
			...
			<div className="mt-12">
				<div className="rounded border bg-card px-4 py-2">
			...
			<div className="divide-y divide-border border-b border-t border-border">
				{[1, 2].map((i) => (
					<div key={i} className="flex py-4">
						<div className="h-24 w-24 rounded-md bg-secondary sm:h-32 sm:w-32" />
